...
export default function CatalogClient({ catalogItems }: { catalogItems: readonly CatalogItem[] }) {
  ...
  return <main>
    <SiteNav language={language} setLanguage={setLanguage} active="items"/>
    <header className="hero" id="top"><div className="eyebrow">{language === 'hu' ? 'Válogatott mágikus ritkaságok Faerûnból' : 'Curated magical rarities from Faerûn'}</div><h1><span className="titleAurora">{language === 'hu' ? 'Mágikus' : 'Magic'}</span><br/><span className="titleCatalog">{language === 'hu' ? 'Tárgyak' : 'Items'}</span></h1><p>{language === 'hu' ? 'Fegyverek, ereklyék és különös portékák egy helyen.' : 'Weapons, relics, and curious wares in one place.'}</p><label className="search"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={language === 'hu' ? 'Keress név, típus vagy hatás szerint…' : 'Search by name, type, or effect…'} aria-label={language === 'hu' ? 'Keresés a tárgyak között' : 'Search items'}/><kbd>{filtered.length} {language === 'hu' ? 'találat' : 'results'}</kbd></label></header>
